...
import json
import boto3    topics_ok = all('EXISTS' in status for status in topic_results.values())
    lambdas_ok = all(result.get('status') == 'OK' for result in lambda_results.values() if 'status' in result)import os
import sys
        ...
        try:
            ...
            for expected_var in expected_vars[function_name]:
                if expected_var in env_vars:
                    topic_name = env_vars[expected_var].split(':')[-1] if ':' in env_vars[expected_var] else env_vars[expected_var]
...
def generate_test_report():
    ...
    lambdas_ok = all(result.get("status") == "OK" for result in lambda_results.values())
