# BuildingOS API Testing Script - Improved Version
...
$directorUri = "$BaseUrl/director?user_request=Test%20from%20PowerShell"
...
# Test 6: Coordinator
$missionId = "powershell-mission-test"
...
if ($Performance) {
    ...
    $avgResponseTime = ($script:results | Measure-Object -Property ResponseTime -Average).Average
    $maxResponseTime = ($script:results | Measure-Object -Property ResponseTime -Maximum).Maximum
    $minResponseTime = ($script:results | Where-Object { $_.ResponseTime -gt 0 } | Measure-Object -Property ResponseTime -Minimum).Minimum
    ...
    $script:results | Select-Object Name, Status, ResponseTime, StatusCode | Format-Table -AutoSize
}

# Failed tests details
$failedTests = $script:results | Where-Object { $_.Status -eq "Failed" }
